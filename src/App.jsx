...
async function gsGet(sheet) {
  try {
    const res = await fetch(`${GS_API}?sheet=${sheet}`);
...
// 把 Sheets 原始列（物件）轉成 App 用的格式
function sheetRowToWisdom(row, idx) {
  return {
    id: idx,
    cat: row["類別 Category"] || "",
    title: row["標題 Title"] || "",
    content: row["內容 Content"] || "",
    star: row["靈魂標記 Star"] || "",
    trigger: row["對應情境 Trigger"] || "",
    custom: false,
    fromSheet: true,
  };
}
...
export default function App() {
  ...
  // Google Sheets 同步狀態
  const [gsWisdom,setGsWisdom]=useState(null); // null=未載入 []=載入中 [..]=完成
  ...
  // 啟動時從 Google Sheets 載入智慧錦囊
  useEffect(()=>{
    setSyncStatus("syncing");
    gsGet("Wisdom_DB").then(rows=>{
      if(rows && rows.length > 0){
        const wisdom = rows.map((r,i)=>sheetRowToWisdom(r,i));
        setGsWisdom(wisdom);
        setSyncStatus("ok");
        setLastSync(new Date().toLocaleTimeString("zh-TW",{hour:"2-digit",minute:"2-digit"}));
      } else {
        setGsWisdom([]); // 空陣列=載入失敗，用內建資料
        setSyncStatus("err");
      }
    });
  },[]);
  ...
  // 智慧錦囊來源：優先用 Google Sheets，否則用內建 WISDOM_SEED
  const customLen = app?.customWisdom?.length ?? 0;
  const gsLen = gsWisdom?.length ?? 0;
  const allWisdom = useMemo(()=>{
    const base = gsWisdom && gsWisdom.length > 0
      ? gsWisdom  // ✅ 來自 Google Sheets
      : WISDOM_SEED.map(([i,c,t,x,s])=>({id:i,cat:c,title:t,content:x,star:s,custom:false})); // 內建備用
    if(!app) return base;
        ...
        <div style={{display:"flex",gap:10,alignItems:"center"}}>
          ...
          <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:5}}>
              <div style={{fontSize:9,color:"var(--c)",fontFamily:"monospace",letterSpacing:".04em"}}>智慧錦囊 · 重寫神經路徑</div>
              {/* 同步狀態指示器 */}
              <div style={{display:"flex",alignItems:"center",gap:3,fontSize:9,color:
                syncStatus==="ok"?"#34d399":syncStatus==="syncing"?"#f59e0b":"#f87171"}}>
                ...
                {syncStatus==="ok"&&`GS ✓ ${lastSync}`}
                {syncStatus==="syncing"&&"連接中…"}
                {syncStatus==="err"&&"離線模式"}
              </div>
            </div>
          </div>
